...
const FaqSection = () => {
  ...
  return (
    <section id="faq" className="py-20 px-4 bg-gradient-to-br from-gray-50 via-blue-50/20 to-purple-50/20 md:py-[80px] relative overflow-hidden">
      {/* Enhanced background elements */}
      <div className="absolute inset-0 bg-gradient-to-r from-gray-100/20 to-blue-100/20 animate-gradient-x"></div>
      <div className="absolute top-1/4 right-1/4 w-96 h-96 bg-blue-200/10 rounded-full blur-3xl animate-float"></div>
      <div className="absolute bottom-1/4 left-1/4 w-96 h-96 bg-purple-200/10 rounded-full blur-3xl animate-float" style={{ animationDelay: '4s' }}></div>

      <div className="container mx-auto max-w-3xl relative z-10">
        <div className="text-center mb-16 animate-fade-in-up">
          <div className="inline-block bg-gradient-to-r from-gray-100 to-blue-100 rounded-full px-6 py-2 text-sm font-medium text-gray-700 mb-5 border border-gray-200/50 shadow-lg backdrop-blur-sm hover:scale-105 transition-all duration-300">
            ❓ Got Questions?
          </div>
        ...
        <div className="space-y-4 animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
          {faqs.map((faq, index) => (
            <div 
              key={index} 
              className="bg-white/70 backdrop-blur-sm rounded-xl overflow-hidden shadow-lg border border-white/20 hover:shadow-xl transition-all duration-500 hover:scale-[1.02] group"
              style={{ animationDelay: `${0.5 + index * 0.1}s` }}
            >
              <button 
                onClick={() => toggleAccordion(index)} 
                className="flex justify-between items-center w-full p-6 text-left hover:bg-white/20 transition-all duration-300 group-hover:bg-gradient-to-r group-hover:from-blue-50/30 group-hover:to-purple-50/30"
              >
                <span className="text-lg font-semibold text-gray-800 pr-4 group-hover:text-blue-800 transition-colors duration-300">
                  {faq.question}
                </span>
              ...
              >
                <div className="px-6 pb-6">
                  <div className="border-t border-gradient-to-r from-blue-200/50 to-purple-200/50 pt-4">
